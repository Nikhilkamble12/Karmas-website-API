...
export default async function defineRoutes(wsRouter, activeConnections) {
  async function checkSosUsers() {
    try {
      ...
      const getAllActiveSosUser = await LocalJsonHelper.getAll(
        "sos_user_list",
        "15d"
      );
      ...
      if (
        ...
        for (const user of getAllActiveSosUser) {
          if (user.is_active) {
            ...
            const userSosStatus = await LocalJsonHelper.getAll(
              activeSos,
              "15d",
              "sos_id",
              user.sos_id
            ...
            if (userSosStatus && userSosStatus.length > 0) {
              ...
              if (
                timeDifference.timeDifferenceInMilliseconds >= reminderThreshold
              ) {
                ...
                if (targetWs && targetWs.readyState === WebSocket.OPEN) {
                  ...
                  const template =
                    notificationTemplates.sosLocationUpdateRequest({
                      username: user.contact_name,
                    });
                  const userTokens = await UserTokenService.GetTokensByUserIds([
                    user.user_id,
                  ]);
